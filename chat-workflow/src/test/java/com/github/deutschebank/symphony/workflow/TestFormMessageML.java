...
public class TestFormMessageML extends AbstractMockSymphonyTest {
	...
	public void testFreemarkerView() throws Exception {
		...
		String actual = messageMlConverter.convert(TestTemplatedObject.class, to4, Collections.singletonList(submit), false, ErrorHelp.createErrorHolder(), empty);
	...
	public void testNewWeirdFieldsEdit() throws Exception {
		...
		Button submit = new Button("submit", Type.ACTION, "GO");
		EntityJson empty = new EntityJson();
		String actual = messageMlConverter.convert(TestOb4.class, to4, Collections.singletonList(submit), true, ErrorHelp.createErrorHolder(), empty);
		System.out.println("<messageML>" + actual + "</messageML>");
	...
	public void testNewWeirdFieldsView() throws Exception {
		Author.CURRENT_AUTHOR.set(new UserDef("sdfjk", "bbb", "v@bkfjd.com"));
		TestOb4 to4 = new TestOb4();
		to4.setB(true);
		to4.setC(Choice.B);
	...
	public void testAxeFormEditMessageML() throws Exception {
		...
		EntityJson empty = new EntityJson();
		String out = messageMlConverter.convert(TestObject.class, a, Collections.singletonList(submit), true,
		...
		out = messageMlConverter.convert(TestObject.class, null, Collections.singletonList(submit), true,
				ErrorHelp.createErrorHolder(), empty);
		System.out.println("<messageML>" + out + "</messageML>");
		Assert.assertEquals(
	...
	public void testAxeFormViewMessageML() throws Exception {
		...
		// can we convert to messageML? (something populated)
		EntityJson empty = new EntityJson();
		String out = messageMlConverter.convert(TestObject.class, a, Collections.singletonList(submit), false,
				ErrorHelp.createErrorHolder(), empty);
		System.out.println("<messageML>" + out + "</messageML>");
	...
	public void testAxesTableEditMessageML() throws Exception {
		...
		// can we convert to messageML? (something populated)
		EntityJson empty = new EntityJson();
		String out = messageMlConverter.convert(TestObjects.class, a, Collections.singletonList(submit), true,
				ErrorHelp.createErrorHolder(), empty);
		System.out.println("<messageML>" + out + "</messageML>");
	...
	public void testAxesTableViewMessageML() throws Exception {
		...
		// can we convert to messageML? (something populated)
		EntityJson empty = new EntityJson();
		String out = messageMlConverter.convert(TestObjects.class, a, Collections.singletonList(submit), false,
				ErrorHelp.createErrorHolder(), empty);
		System.out.println("<messageML>" + out + "</messageML>");
	...
	public void testValidation() throws Exception {
		...
		EntityJson empty = new EntityJson();
		String out = messageMlConverter.convert(TestObject.class, a, Collections.singletonList(submit), true, eh, empty);
		System.out.println("<messageML>" + out + "</messageML>");
