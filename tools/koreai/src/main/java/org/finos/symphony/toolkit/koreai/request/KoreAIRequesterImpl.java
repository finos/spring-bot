...
public class KoreAIRequesterImpl implements KoreAIRequester, InitializingBean {
	...
	public KoreAIRequesterImpl(KoreAIResponseHandler koreaiResponseParser, KoreAIResponseBuilder koreAIResponseBuilder,
			String url, JsonNodeFactory jsonNodeFactory, String jwt) {
	...
	protected Client createClient() {
		...
		return ClientBuilder.newBuilder().connectTimeout(15, TimeUnit.SECONDS).readTimeout(15, TimeUnit.SECONDS)
				.build();
	}
	...
	public Object buildPayload(Address a, String text) {
		...
		payload.set("from", jnf.objectNode().put("id", String.valueOf(a.getUserId())).set("userInfo", jnf.objectNode()
				.put("firstName", a.getFirstName()).put("lastName", a.getLastName()).put("email", a.getEmail())));
