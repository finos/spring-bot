...
<#-- starting template -->
<form 
  id="org.finos.symphony.toolkit.workflow.fixture.RoomAndDropdown">
 <table>
 <tr><td><b>Chat:</b></td><td>
  <span class="tempo-text-color--red">${entity.errors.contents['chat']!''}</span>
  <select 
   name="chat"
 ...
 <tr><td><b>Dyn Options:</b></td><td>
  <span class="tempo-text-color--red">${entity.errors.contents['dynOptions']!''}</span>
  <select 
   name="dynOptions"
   data-placeholder="Choose dyn options">
    <#list entity.dynoptions?keys as iB>
     <option value="${iB}" selected="${(entity.form.dynOptions == iB)?string('true','false')}">
      ${entity.dynoptions[iB]}
     </option>
   </#list>
  </select></td></tr>
 </table>
