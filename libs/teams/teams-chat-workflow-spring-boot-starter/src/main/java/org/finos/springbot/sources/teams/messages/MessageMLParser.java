...
import org.finos.springbot.sources.teams.content.RoomName;
...
public class MessageMLParser {
	...
	static class TagFrame<X extends Tag> extends TextFrame<X> {
		...
		public X getContents() {
			if ((contents instanceof TeamsUser) && (contents.getName()==null)) {
				TeamsUser su = (TeamsUser) contents;
				su.getId().add(new DisplayName(bufferWithoutPrefix()));
			} else if ((contents instanceof TeamsChat) && (contents.getName() == null)) {
				TeamsChat sr = (TeamsChat) contents;
			...
			saxParser.parse(new InputSource(new StringReader(message)), new DefaultHandler2() {
				...
				public void startElement(String uri, String localName, String qName, Attributes attributes)
					...
					if (top instanceof CodeBlockFrame) {
						...
						if (o instanceof TeamsUser) {
							push(new TagFrame<TeamsUser>((TeamsUser) o));
